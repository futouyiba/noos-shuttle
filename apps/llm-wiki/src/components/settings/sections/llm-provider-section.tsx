        ...
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          {state === "ok" && (
            <>
              <div>
                Detected{result?.version ? ` ${result.version}` : ""}. Ready to use your local
                Codex binary — no API key needed.
              </div>
              {result?.path && (
                <div className="truncate font-mono text-[10px] text-muted-foreground">
                  {result.path}
              ...
              <div className="text-muted-foreground">
                LLM Wiki uses an isolated Codex login so it will not disturb Codex App sessions. If
                chat fails with an authentication error, run{" "}
                <code className="rounded bg-background/60 px-1 py-0.5 font-mono text-[10px]">
                  CODEX_HOME=&lt;LLM Wiki app data&gt;/codex-cli-home codex login --device-auth
                </code>{" "}
                in a terminal to refresh this provider's login.
              </div>
            </>
          )}
          {state === "err" && (
