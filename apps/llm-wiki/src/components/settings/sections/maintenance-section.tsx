...
import { enqueueAllSourceIngest } from "@/lib/source-lifecycle"
...
export function MaintenanceSection() {
  ...
  const handleRebuildSources = useCallback(async () => {
    if (!project) return
    setRebuilding(true)
    setRebuildError(null)
    setRebuildCount(null)
    try {
      const ids = await enqueueAllSourceIngest(project, llmConfig, {
        clearCache: true,
      })
      setRebuildCount(ids.length)
    } catch (err) {
      ...
      <div className="space-y-3 rounded-lg border border-border/60 bg-muted/20 p-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">
            {t("settings.sections.maintenance.rebuild.title", {
              defaultValue: "Rebuild source analysis",
            })}
          </h3>
        </div>
        <p className="text-xs leading-relaxed text-muted-foreground">
          {t("settings.sections.maintenance.rebuild.description", {
            defaultValue:
              "Re-run analysis and wiki generation for every existing file under raw/sources without importing files again. This clears ingest cache entries first, then queues the normal ingest pipeline so summaries, index, overview, concepts, and entities can be refreshed.",
          })}
        </p>
        ...
        <Button
          onClick={() => void handleRebuildSources()}
          disabled={rebuilding || !projectReady || !llmReady}
        >
          {rebuilding ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("settings.sections.maintenance.rebuild.queueing", {
            ...
            <>
              ...
              {t("settings.sections.maintenance.rebuild.button", {
                defaultValue: "Rebuild all sources",
              })}
            </>
          )}
        </Button>

        {rebuildCount !== null && !rebuildError && (
          <div className="flex items-start gap-1.5 rounded border border-emerald-500/40 bg-emerald-500/5 px-2 py-1.5 text-xs text-emerald-700 dark:text-emerald-400">
            ...
            <div>
              {t("settings.sections.maintenance.rebuild.queued", {
                defaultValue: "Queued {{n}} source file(s) for rebuild.",
                n: rebuildCount,
              })}
            </div>
          </div>
