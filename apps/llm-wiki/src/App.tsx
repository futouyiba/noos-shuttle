...
interface SleepRecoveryStatus {
  ...
  lastReason: string
  lastResumeEpoch?: number
  lastGapSecs?: number
  relaunchRecommended?: boolean
  message: string
}

async function markFrontendSleepRecovery(failed: boolean, reason: string) {
  try {
    await invoke<SleepRecoveryStatus>(
      failed ? "mark_sleep_recovery_degraded" : "mark_sleep_recovery_healthy",
      { reason }
    )
  } catch (err) {
    console.warn("[sleep-recovery] failed to update recovery status:", err)
  }
}
...
function App() {
  ...
  useEffect(() => {
    let cancelled = false
    let unlistenSuspended: (() => void) | null = null
    let unlistenResumed: (() => void) | null = null

    async function installSleepRecoveryListeners() {
      try {
        unlistenSuspended = await listen("tauri://suspended", () => {
          void invoke<SleepRecoveryStatus>("mark_sleep_suspended")
        })
        unlistenResumed = await listen("tauri://resumed", () => {
          void recoverAfterSleep("frontend tauri resumed event")
        })
      } catch (err) {
        console.warn("[sleep-recovery] failed to install listeners:", err)
      }
    }

    async function recoverAfterSleep(reason: string) {
      if (cancelled) return
      let recoveryFailed = false
      try {
        await invoke<SleepRecoveryStatus>("recover_from_sleep", {
          reason,
          gapSecs: null,
        })
      } catch (err) {
        console.warn("[sleep-recovery] backend marker failed:", err)
        recoveryFailed = true
      }
      ...
      if (!current || cancelled) {
        await markFrontendSleepRecovery(recoveryFailed, "frontend recovery finished without an open project")
        return
      }

      try {
        const { restoreQueue } = await import("@/lib/ingest-queue")
        await restoreQueue(current.id, current.path)
      } catch (err) {
        console.warn("[sleep-recovery] failed to restore ingest queue:", err)
        recoveryFailed = true
      }
      ...
      try {
        ...
        console.warn("[sleep-recovery] failed to recover project file sync:", err)
        recoveryFailed = true
      }
      ...
      try {
        ...
        console.warn("[sleep-recovery] failed to recover scheduled import:", err)
        recoveryFailed = true
      }

      try {
        const tree = await listDirectory(current.path)
        useWikiStore.getState().setFileTree(tree)
        useWikiStore.getState().bumpDataVersion()
      } catch (err) {
        console.warn("[sleep-recovery] failed to refresh project tree:", err)
        recoveryFailed = true
      }

      await markFrontendSleepRecovery(recoveryFailed, "frontend watcher, queue, import, and tree recovery")
    }
