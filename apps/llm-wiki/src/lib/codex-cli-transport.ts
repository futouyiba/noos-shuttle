...
export function buildPrompt(messages: ChatMessage[]): string {
  const preamble = [
    "You are running as a plain text LLM backend for LLM Wiki.",
    "Do not inspect files, run shell commands, call tools, modify the workspace, or look for repository context.",
    "Use only the messages below as context and produce the requested final text output.",
  ].join("\n")
...
export async function streamCodexCli(
  ...
  try {
    ...
    await new Promise<void>((resolve) => {
      settle = resolve
      void invoke("codex_cli_spawn", payload).catch((err) => {
        finishWith(() => {
