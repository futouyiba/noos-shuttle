...
export async function enqueueSourceIngest(
  ...
  sourcePaths: string[],
  llmConfig: LlmConfig,
  options: { sourceRoot?: string; rootContext?: string; clearCache?: boolean } = {},
): Promise<string[]> {
  ...
  if (options.clearCache) {
    for (const file of files) {
      const fileName = getFileName(file.sourcePath)
      if (!fileName) continue
      try {
        await removeFromIngestCache(pp, fileName)
      } catch {
        // cache invalidation is best effort; enqueue should still proceed
      }
    }
  }
...
export async function enqueueAllSourceIngest(
  project: WikiProject,
  llmConfig: LlmConfig,
  ...
  const tree = await listDirectory(sourcesRoot)
  const sourcePaths = flattenFiles(tree)
    .map((node) => node.path)
    .filter(isIngestableSourcePath)
