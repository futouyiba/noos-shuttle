...
权威展开见 noos_docs 当前 canonical；需要精确复现时使用 commit SHA。
本表只提供入口：
...
- **路径交集检查是「证据 / 风险信号」，不是门**：比较分支相对 merge-base 的净
  ...
  两侧路径不相交即互不覆盖）。**交集非空**同样是信号（提示存在语义耦合面），须
  核对合并结果或改走窄迁移。**该证据本身有边角**：比较**按路径名**进行、**不覆盖
  重命名**——分支重命名 `A→B` 而 main 改了旧名 `A` 时，两侧名字不同、可能读到
  **空交集**，而耦合面实际存在；此情形 git 以**冲突**暴露（modify/delete），**并非
  静默回退**，故上文「不会回退」仍成立，但**「交集为空」不得被读成「不存在耦合
  面」**。
