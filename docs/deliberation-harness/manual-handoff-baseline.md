...
>
> Successor to `cross-agent-handoff-manual-dogfood-baseline-v0.md` (2026-09-15): that note captured how the loop was *discovered* (traces A/B, pre-spec clipboard era). This note captures how the loop *actually runs today*, after the cross-conversation workflow spec (`futouyiba/noos_docs` `docs/agent-workflow.md`, v0.3.x, 2026-09-16/17) and its Appendix B cipher/marker protocol were adopted and dogfooded.
>
> Method: every step is anchored to real dogfood events on this repository (issue/PR/comment/commit refs). Nothing here is aspirational. Verbatim quotes are marked as such.

## 1. What changed since the v0 baseline

| Dimension | v0 era (≤2026-09-15) | Now (2026-09-17) |
| --- | --- | --- |
| Durable coordination | ad-hoc: chat transcripts + pasted packets | GitHub issue/PR threads; machine-written markers (`REVIEW:` / `DESIGN:` / `IMPLEMENTED:` / `INTEGRATED:`) with provenance lines |
| Trigger transport | human types full instructions | cipher triggers (`dispatch` / `implement #N` / `review PR#N` / `merge PR#N` / `fix PR#N`), pointers only, spec Appendix B |
| Impl-side routing | human copy/paste between windows | machine-carried: cross-session messages between local Claude Code sessions (incl. automation sessions dispatching over UDS) |
...
6. **Integrate.** Resident integrator verifies marker/provenance/delegation/three-way head identity, archives review intake locally (`.review-intake/`, not committed), merges, re-runs verification, builds/deploys where applicable, posts `INTEGRATED: <summary + build timestamp> @ <merge-sha>` (PR `#43` comment `5705431032`; PR `#42` marker `@ 990037c`).
...
Amendment loops on the contract line ran as focused re-reviews anchored at exact SHAs (issues `#15`/`#16`: N1–N5 and M1–M3 amendment rounds).
...
PR `#38`: round-1 REQUEST_CHANGES @ `3537da9` → implementer disposition comment (`5704608127`) → round-2 incremental APPROVE @ `f874766` (`5704772633`). Any commit after a reviewed head re-triggers incremental review (spec §1.3).
...
## 5. What the human still does (updated from v0 §5)
...
1. **Single GitHub identity cannot formally approve.** "GitHub rejects a formal APPROVE on one's own pull request" — noted verbatim in two PR `#34` review submissions and in the issue `#35` review record; verdicts therefore ride comment markers. Spec B.3 acknowledges this: markers are an audit protocol, not a security boundary.
