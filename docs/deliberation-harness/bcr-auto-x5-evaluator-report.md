...
```text
Human starts Go ×5 (AUTO badge; no goal prompt — see the default continuation contract in §7). Round flow:→ GO via SubmissionOperation → stable Assistant Turn (Slice 1, unchanged)
...
- **Bounded envelope** (contract §8): evaluation basis = the run's optional Human-authored goal, else the built-in default continuation contract (§7); scope (defaults to the goal basis), completed assistant turn tail (last ≤2000 chars — the tail, because stop-boundary phrasing and the veto window live at the end of a turn), no fabricated checkpoint/frontier. The envelope is assembled background-side from the durable run; the wire only carries `runId` + excerpt (≤8000 chars), and the evaluate lane additionally refuses any run whose durable phase is not `EVALUATING`.
...
Rounds 1–3 submit the plain `go`. Rounds 4–5 prepend a bounded `[NOOS Re-anchor]` block restating the run's evaluation basis (Human-authored goal, or the built-in default continuation contract in Chinese) and the scope guard (`max_plain_go_streak = 3`, policy parameter, not invariant). Provenance stays distinct: the transport payload is Harness-generated and each round's evidence candidate records `continuationMode: PLAIN_GO | REANCHOR_GO`. No Goal/Scope mutation occurs; re-anchor is context maintenance only.
...
- Content UI: settings panel gains "BCR 自动评估（实验）" (DeepSeek API key — write-only, and model fields; saved via the config lane). With a configured evaluator the BCR panel shows the AUTO ×5 badge — no goal prompt (§7 default contract). Active AUTO runs show `EVALUATING` phase and always-available Stop; reload mid-evaluation resumes the evaluation (adoption path); a READY_TO_GO resume after a crash surfaces the manual [Send go] button rather than silently re-dispatching.
...
- `tests/continuation-run.test.ts` (28): AUTO_X5 start validation (goal/scope optional-but-validated), COMPLETED → EVALUATING routing per mode, pass/stop advance mapping, budget-exhaustion precedence over evaluation, phase guards (no HUMAN_CONTINUE/dispatch during EVALUATING), evidence with continuationMode + assessment.
...
2. `AUTO_DOGFOOD_PENDING` → **satisfied 2026-09-17 (§7)**: a fully automatic ×5 run consumed its whole budget with zero per-round Human clicks. Ongoing acceptance now rides each new live AUTO run's durable candidates.
